...
const Questions = () => {
  ...
  const submitAnswer = async (e) => {
    ...
    {
      setDisable(true);
      const response = await axios.post('http://localhost:4000/matchAnswer', {
      ...
      if(result.verified == true)
      {
        ...
        const response2 = await axios.post('http://localhost:4000/updateLeaderboard',{
          access_token: import.meta.env.VITE_ACCESS_TOKEN,
          qNo: qNo,
          enrollment: enroll
        }, {
  ...
  useLayoutEffect(() => {
    (async ()=>{
      if(Cookies.get('loggedIn') == "true" && Cookies.get('sessionToken').length > 0)
      {
        ...
        try
        {
          const response = await axios.post('http://localhost:4000/fetchUserProgress', {
            access_token: import.meta.env.VITE_ACCESS_TOKEN,
          ...
          else
          {
            ...
            try{
              const response2 = await axios.post('http://localhost:4000/returnQuestion', {
                access_token: import.meta.env.VITE_ACCESS_TOKEN,
